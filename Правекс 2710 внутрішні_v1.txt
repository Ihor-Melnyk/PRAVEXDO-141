...
function setOrgaResponsible1() {
  var state = EdocsApi.getCaseTaskDataByCode("ResponsibleTask")?.state;
  debugger;
  if (state == "assigned" || state == "inProgress" || state == "delegated") {
    var Responsible = EdocsApi.getAttributeValue("Responsible");
    if (Responsible.value) {
      EdocsApi.setAttributeValue({
        code: "OrgaResponsible1",
        value: JSON.parse(Responsible.value)[0].employeeId,
      });
    } else {
      EdocsApi.setAttributeValue({
        code: "OrgaResponsible1",
